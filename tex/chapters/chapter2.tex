...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\linewidth]{tex/figures/selected/figure01-flow.png}
  \caption{Flow 可以看成向量场驱动下对整个空间的连续变形。蓝色箭头表示速度场，红色网格表示空间在不同时刻被推送后的形状。}
  \label{fig:flow-vector-field}
\end{figure}

图~\ref{fig:flow-vector-field} 给出了二维情形下的直观图像：向量场规定每个位置的瞬时速度，而 flow 则记录这些速度如何把空间整体变形。
...
假设现在又一个神经网络参数化的向量场 $u_t^\theta$，我们的目标是从数据分布 $\data$ 中生成（采样）得到 $z$。这些样本需要是随机的，但由于ODE本身在确定初始状态和向量场后是完全确定的，因此我们需要确保初始状态随机。我们选择一个初始分布 $p_{\mathrm{init}}=\mathcal{N}(0,I_d)$。我们因此可以通过ODE来描述一个flow model
\begin{equation}
  \begin{aligned}
...
我们的目标是让轨迹终点 $X_1$ 服从数据分布 $\data$，即 $X_1=\psi_1^\theta(X_0) \sim \data$。为了计算flow，我们可以Euler法数值模拟ODE得到最终的数据分布
...
\State Draw a sample $X_0 \sim p_{\mathrm{init}}$
\For{$i = 1, \dots, n$}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item 布朗运动可以用如下方式模拟：
    \begin{equation}
      W_{t+h}=W_t+\sqrt{h}\, \varepsilon_t, \qquad \varepsilon_t \sim \mathcal{N}(0,I_d)
    \end{equation}
  \end{itemize}
\end{itemize}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\linewidth]{tex/figures/selected/figure03-ou.png}
  \caption{Ornstein--Uhlenbeck 过程的样本轨迹。不同扩散强度会产生不同程度的随机波动。}
  \label{fig:ou-process}
\end{figure}

图~\ref{fig:ou-process} 说明了 SDE 中 drift 和 diffusion coefficient 的共同作用：drift 将轨迹拉向稳定区域，而随机项带来持续扰动。

和ODE一样，我们的目标是将简单的初始分布 $p_{\mathrm{init}}$ 映射到复杂的真实数据分布 $\data$。我们同样通过神经网络参数化向量场 $u_t$。我们用SDE来描述一个diffusion model
\begin{equation}
  \begin{aligned}
    X_0 &\sim p_{\mathrm{init}}
  & \qquad\qquad\qquad\qquad\qquad \blacktriangleright \: \makebox[3cm][l]{\text{随机初始化}} \\
