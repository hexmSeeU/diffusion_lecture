...
\right].
\end{equation}
其中采样 $\vz\sim\data,\vx\sim p_t(\cdot|\vz)$ 等价于先从边缘分布
$p_t$ 采样 $\vx$。
...
\end{equation}
从 $p_t(\cdot|\vz)$ 采样。由前面的计算，条件 score 满足
\begin{equation}
\nabla_\vx\log p_t(\vx|\vz)
=
-\frac{\vx-\alpha_t\vz}{\beta_t^2}
...
\)，令
\begin{equation}
  -\beta_t s_t^\theta(\vx)=\epsilon_t^\theta(\vx),
\end{equation}
并丢掉整体的 $\frac{1}{\beta_t^2}$ 权重，得到常用的噪声预测目标
\begin{equation}
\mathcal{L}_{\mathrm{DDPM}}(\theta)
=
\E_{t\sim \mathrm{Unif}[0,1],\,\vz\sim\data,\,\epsilon\sim\Normal(0,I_d)}
\left[
\left\|
\epsilon_t^\theta(\alpha_t\vz+\beta_t\epsilon)-\epsilon
\right\|^2
\right].
\end{equation}
\end{example}
...
\begin{algorithmic}[1]
\Require Dataset samples $\vz\sim\data$, score network $s_t^\theta$ or noise predictor $\epsilon_t^\theta$
\For{each mini-batch}
    \State Sample data $\vz$ from the dataset
    \State Sample time $t\sim \mathrm{Unif}[0,1]$
    \State Sample noise $\epsilon\sim\Normal(0,I_d)$
    \State Set $\vx_t=\alpha_t\vz+\beta_t\epsilon$
    \State Compute score matching loss
    \[
    \mathcal{L}(\theta)
    =
    \left\|
    s_t^\theta(\vx_t)+\frac{\epsilon}{\beta_t}
    \right\|^2
    \]
...
\begin{summarybox}
设 $p_t(\vx|\vz)$ 和 $p_t(\vx)$ 分别为条件概率路径和边缘概率路径。
条件 score 与边缘 score 分别定义为 $\nabla_\vx\log p_t(\vx|\vz)$ 和 $\nabla_\vx\log p_t(\vx)$。
...
\[
  p_t(\cdot|\vz)=\Normal(\alpha_t\vz,\beta_t^2I_d),
\]
