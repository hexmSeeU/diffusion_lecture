...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.72\linewidth]{tex/figures/selected/figure08-score.png}
  \caption{Score function 指向 log-likelihood 增长最快的方向。左侧是概率密度，右侧箭头表示对应的 score field。}
  \label{fig:score-function}
\end{figure}

图~\ref{fig:score-function} 给出了 score 的几何直觉：低密度区域的点会沿着 score 指向更高密度区域。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\linewidth]{tex/figures/selected/figure09-sde-paths.png}
  \caption{用 SDE 模拟同一条概率路径。随机噪声使单条轨迹变得曲折，但边缘分布仍沿着指定的 $p_t$ 演化。}
  \label{fig:sde-path-simulation}
\end{figure}

图~\ref{fig:sde-path-simulation} 对比了 SDE 采样与 ODE 采样的效果：SDE 轨迹具有随机性，但整体样本分布仍保持在目标概率路径上。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\linewidth]{tex/figures/selected/figure10-langevin.png}
  \caption{Langevin dynamics 的直观效果：粒子在 score 引导和随机噪声共同作用下逐渐接近目标分布。}
  \label{fig:langevin-dynamics}
\end{figure}

图~\ref{fig:langevin-dynamics} 展示了 score 与随机噪声共同塑造采样分布的典型例子；这也是 diffusion model 中 SDE 采样直觉的重要来源。
